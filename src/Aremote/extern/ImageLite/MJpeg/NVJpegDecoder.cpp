...
namespace ImageLite
{
	namespace JpegGpu
	{
		...
		NVJpegDecoder::NVJpegDecoder(int32_t did)
		{
			nvparam = std::make_unique<gpuparams_t>();
			nvparam->did = did;
			init();
		...
		void NVJpegDecoder::imginfo(std::vector<uint8_t> const& data)
		{
		...
		bool NVJpegDecoder::imgstream(std::vector<uint8_t> const& idata, std::vector<uint8_t>& odata)
		{
			if (!nvisenable)
				return false;
		...
		bool NVJpegDecoder::imgread(std::string const& s, std::vector<uint8_t>& odata)
		{
			if (!nvisenable)
				return false;
		...
		void NVJpegDecoder::imgdecompress(std::vector<uint8_t> const& idata, std::vector<uint8_t>& odata)
		{
			try
			{
		...
		void NVJpegDecoder::imgbuffer(std::vector<uint8_t>& odata)
		{
			int32_t const sz = (imgparam.height * imgparam.width);
			std::vector<uint8_t> Rc(sz);
			std::vector<uint8_t> Gc(sz);
			...
			if (Helper::intrin.getcpuid() == Helper::Intrin::CpuId::CPU_AVX2)
				tobuffer_avx2(Rc, Gc, Bc, sz, imgparam, odata, error);
			else
